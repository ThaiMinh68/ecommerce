...
if (isset($_GET['id']) && ctype_digit($_GET['id'])) {
    ...
    $stmt = mysqli_prepare($db, "SELECT p.*, c.category_name, GROUP_CONCAT(pi.filename) AS images
        FROM Products p
...
$mainImage = !empty($images[0]) ? 'assets/images/products/' . trim($images[0]) : 'assets/images/homepage-one/product-img/product-img-14.webp';
$productName = htmlspecialchars($product['product_name']);
$productShortDescription = htmlspecialchars($product['short_description'] ?? $product['description'] ?? '');
$productPrice = number_format((float)$product['price'], 2);
$productStock = (int)($product['stock'] ?? 0);
$categoryName = htmlspecialchars($product['category_name'] ?? 'Unknown');
?>
                ...
                <div class="row ">
                                    ...
                                    <?php if (!empty($images)): ?>
                                        <?php foreach ($images as $img): ?>
                                            <div class="swiper-slide slider-top-img">
                                                <img src="assets/images/products/<?php echo htmlspecialchars(trim($img)); ?>" alt="<?php echo $productName; ?>">
                                            </div>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <div class="swiper-slide slider-top-img">
                                            <img src="<?php echo htmlspecialchars($mainImage); ?>" alt="<?php echo $productName; ?>">
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                    ...
                    <div class="col-md-6">
                        <div class="product-info-content" data-aos="fade-left">
                            <span class="wrapper-subtitle"><?php echo $categoryName; ?></span>
                            ...
                            <p class="content-paragraph"><?php echo $productShortDescription; ?></p>
                            <hr>
                            <div class="product-availability">
                                <span>Availability : </span>
                                <span class="inner-text"><?php echo $productStock > 0 ? $productStock . ' products available' : 'Out of stock'; ?></span>
                            </div>
                            ...
                            <div class="product-quantity">
                                <form action="cart.php?action=add" method="POST" style="display:flex; gap:15px; align-items:center; margin:0;">
                                    <input type="hidden" name="id" value="<?php echo $product['id']; ?>">
                                    <div class="quantity-wrapper">
                                        <div class="quantity">
                                            <span class="minus" onclick="document.getElementById('qty_input').stepDown()">-</span>
                                            <input type="number" id="qty_input" class="number" name="qty" value="1" min="1" max="<?php echo $product['stock']; ?>" style="width: 30px; text-align: center; border: none; background: transparent; font-weight: 600; padding:0; outline: none; -moz-appearance: textfield;">
                                            <span class="plus" onclick="document.getElementById('qty_input').stepUp()">+</span>
                                        </div>
                                        <div class="wishlist">
                                            <span>
                                    ...
                                    <button type="submit" class="shop-btn" style="border: none; outline: none; min-width: 140px; display: flex; align-items: center; justify-content: center; gap: 8px;">
                                        <span>
                            ...
                            <div class="product-details">
                                <p class="category">Category : <span class="inner-text"><?php echo $categoryName; ?></span></p>
                                <p class="tags">Tags : <span class="inner-text">N/A</span></p>
                                <p class="sku">SKU : <span class="inner-text">#<?php echo $product['id']; ?></span></p>
                            </div>
                            <hr>
